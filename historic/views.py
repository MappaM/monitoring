...
def house(request):
    ...
    power_meters = Meter.objects.select_related('energy','house').filter(house=request.session['house'],appliance_link__isnull=True).exclude(energy__type=Energy.TYPE_STATE).annotate(num_r=Count('readingdays')).exclude(num_r=0)
